...
void mostra_produto();
void consulta_vendas_pagamento();
void consulta_vendas_cpf();
void pagamento();
    ...
    if (strcmp(ptr_venda->cpf, cpf) == 0 && ptr_venda->pago == 'N') {
        ...
        if (resposta == 'S' || resposta == 's') {
            ...
            if (!cliente_encontrado) {
                while (fread(&cliente_atual, sizeof(cliente), 1, fptr_clientes) == 1) {
                    cliente *ptr_cliente = &cliente_atual;

                    if (strcmp(ptr_cliente->CPF, ptr_venda->cpf) == 0) {
                        cliente_encontrado = 1;
                        ptr_cliente->gasto += ptr_venda->total;
                        fseek(fptr_clientes, -sizeof(cliente), SEEK_CUR);
                        fwrite(ptr_cliente, sizeof(cliente), 1, fptr_clientes);
                        break;
                    }
                }
            }
